...
| `term_consistency` | 术语翻译不一致、不应翻译的术语被错误翻译。需翻译的术语（original ≠ translation）使用子串匹配（case-insensitive，长度 ≥ 3），短术语和 do-not-translate 术语使用精确匹配 | 按术语表修正 |
...
| Check | 说明 | 是否强校验 |
|-------|------|-----------|
| `term_consistency` | 术语一致性检查。do-not-translate 术语精确匹配；需翻译术语使用子串匹配（len ≥ 3, case-insensitive），短术语精确匹配 | 是 |
