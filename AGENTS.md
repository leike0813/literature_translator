...
6. 根据一定的 token 阈值对 block 进行批次分割，尽量让各个批次的文本量接近
7. 委派 subagent 执行翻译，让各 subagent 通过质量门禁脚本提交翻译结果；质量门禁脚本主要检验语言正确性、句数一致性、是否有偷懒翻译（校验原文和译文的 token 级长度是否差异过大）、术语一致性问题；未通过质量门禁则打回重做，通过则将结果写盘
8. 主 agent 收集翻译结果并复核
9. 复核完毕后，执行脚本进行译文拼接
